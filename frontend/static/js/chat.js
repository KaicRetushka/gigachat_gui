...
const poleSettings = document.getElementById("poleSettings")
const textarea = document.querySelector("textarea")

srcChats.scrollTop = -srcChats.scrollHeight
allMesages.scrollTop = allMesages.scrollHeight
...
btnSendQuestion.addEventListener("click", async () => {
    ...
    if(listHref.length == 4){
        ...
        srcChats.innerHTML += 
        `<div class="flex justify-between py-3 items-center hover:cursor-pointer" id="titleChat${response.chat_id}">
        ...
        response = await fetch(`/message/${listHref[listHref.length - 1]}`, {
            method: "POST",
            headers: {"content-type": "application/json"},
            body: JSON.stringify({
                text: vlaueTextareaQuestion
...
btnShowChats.addEventListener("click", () => {
    ...
    poleSettings.classList.remove("-left-10/12")
...
function closeChats(){
    ...
    poleSettings.classList.add("-left-10/12")
...
textarea.addEventListener("focus", () => {
    setTimeout(() => {
        textarea.scrollIntoView({behavior: "smooth",top: document.body.scrollHeight});
    }, 200)
})