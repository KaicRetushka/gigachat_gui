...
from backend.database.requests import check_chat, select_all_messages, select_all_chats, select_login
...
async def give_new_chat(request: Request):
    ...
    return templates.TemplateResponse("chat.html", {"request": request, "is_history": False, "list_chats": list_chats, "login": login})
...
async def chat_id(request: Request, chat_id: str):
    ...
    if resposne_db:
        ...
        list_chats = select_all_chats(int(user_id))
        login = select_login(int(user_id))
        return templates.TemplateResponse("chat.html", {"request": request, "messages": messages, "is_history": True, "list_chats": list_chats, "login": login})
